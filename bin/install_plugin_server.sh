...
REMOTE_CMD=$(
  printf "set -euo pipefail; restart_openclaw_gateway() { if command -v openclaw >/dev/null 2>&1; then openclaw_path=\"\$(command -v openclaw)\"; echo \"[INFO] restarting via \$openclaw_path\"; if PATH=\"\$(dirname \"\$openclaw_path\"):\$PATH\" openclaw gateway restart; then return; fi; echo '[WARN] openclaw CLI restart failed, fallback to next strategy'; fi; for candidate in \"\$HOME/.nvm/versions/node\"/*/bin/openclaw \"\$HOME/.local/bin/openclaw\" \"/usr/local/bin/openclaw\"; do if [ -x \"\$candidate\" ]; then echo \"[INFO] restarting via \$candidate\"; if PATH=\"\$(dirname \"\$candidate\"):\$PATH\" \"\$candidate\" gateway restart; then return; fi; echo \"[WARN] restart failed via \$candidate, continue fallback\"; fi; done; if command -v systemctl >/dev/null 2>&1; then echo '[INFO] restarting via systemctl --user openclaw-gateway.service'; systemctl --user restart openclaw-gateway.service; return; fi; echo '[WARN] openclaw command not found and systemctl unavailable; 请远端手动重启'; }; cd %q; WS_URL=%q CHANNEL_ID=%q DISPLAY_NAME=%q DEVICE_ID=%q CAPABILITIES=%q CHANNEL_TAGS=%q REGISTER_TOKEN=%q HEARTBEAT_SEC=%q RECONNECT_MAX_SEC=%q MAX_CONCURRENT_INVOKES=%q QUEUE_WAIT_TIMEOUT_MS=%q INVOKE_IDLE_TIMEOUT_MS=%q STREAM_BUBBLE_SPLIT_GAP_MS=%q OPENCLAW_GATEWAY_URL=%q OPENCLAW_SESSION_NAMESPACE=%q LOCAL_SAFE_GUARD=0 bash bin/install_plugin_local.sh < /dev/null; if [ %q = '1' ]; then rm -rf %q; bash skills/miaochat_file_output/install_skill.sh --no-legacy-link --target-dir %q --api-base %q --user-id %q --token %q; fi; restart_openclaw_gateway" \
    "${REMOTE_DIR}" \
    "${WS_URL}" \
    "${CHANNEL_ID}" \
    "${DISPLAY_NAME}" \
