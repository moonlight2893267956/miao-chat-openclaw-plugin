...
REMOTE_CMD=$(
  printf "set -euo pipefail; cd %q; WS_URL=%q CHANNEL_ID=%q DISPLAY_NAME=%q DEVICE_ID=%q CAPABILITIES=%q CHANNEL_TAGS=%q REGISTER_TOKEN=%q HEARTBEAT_SEC=%q RECONNECT_MAX_SEC=%q MAX_CONCURRENT_INVOKES=%q QUEUE_WAIT_TIMEOUT_MS=%q STREAM_BUBBLE_SPLIT_GAP_MS=%q OPENCLAW_GATEWAY_URL=%q OPENCLAW_SESSION_NAMESPACE=%q LOCAL_SAFE_GUARD=0 bash bin/install_plugin_local.sh < /dev/null; if command -v openclaw >/dev/null 2>&1; then openclaw gateway restart; else echo '[WARN] openclaw command not found; 请远端手动重启'; fi" \
    "${REMOTE_DIR}" \
    "${WS_URL}" \
    "${CHANNEL_ID}" \
    "${DISPLAY_NAME}" \
